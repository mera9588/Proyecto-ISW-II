...
class CutData {
	...
	public static function DayCut(){
		...
		if($breakp>1){
			...
			echo "<p class='alert alert-info'><i class='glyphicon glyphicon-time'></i> Se le recuerda que este es el ultimo día que se trabajara con el corte actual. A partir de el día de mañana todas las operaciones de entrada-salida se reiniciaran de nuevo. </p>";

		}else if($breakp==0){
			echo "<p class='alert alert-warning'><i class='glyphicon glyphicon-time'></i> Se le recuerda que se esta trabajando sobre un nuevo corte, todas las nuevas operaciones seran registradas en el nuevo corte a partir de hoy.</p>";
		}
	...
	public static function getCut(){
		...
		if(self::getCurrent()!=null){
			...
			echo "<p class='alert alert-warning'>Bienvenido al sistema, para iniciar debes crear un cortes, se ha configurado para que los cortes se creen automaticamente todos los días <b>1ro</b> de cada Mes.</p>";
		}
	}
